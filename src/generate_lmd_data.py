...
def tones_to_scales(tones):
    ...
    """
    counts = {tone: {scale: 0 for scale in all_scales} for tone in base_tones}
    ...
    for base_tone in counts:
        ...
        for scale_label in counts[base_tone]:
            frequency[base_tone][scale_label] = counts[base_tone][scale_label] / len(
                tones
            )
    return frequency
...
def normalize_song(tones):
    base_tone, scale_type, scale_score = maximum_likelihood_scale(
        [t for t in tones if t != REST_NOTE]
    )
...
def find_forward(melody_idx, lyric_idx, start, length=250):
    ...
    for i in range(start + 1, len(melody_idx)):
        if (
            melody_idx[i] - melody_idx[start] > length
            or lyric_idx[i] - lyric_idx[start] > length
        ):
            i -= 1
            break
    if i == start:
...
def find_backward(melody_idx, lyric_idx, start, length=500):
    ...
    for i in range(start - 1, -1, -1):
        if (
            melody_idx[start] - melody_idx[i] > length
            or lyric_idx[start] - lyric_idx[i] > length
        ):
            i -= 1
            break
    if i == start:
...
def cut_window(melody, lyric, max_window_size=500):
    ...
    while tail <= len(lyric_idx):
        ...
        if (
            melody_idx[tail] - melody_idx[head] > max_window_size
            or lyric_idx[tail] - lyric_idx[head] > max_window_size
        ):
            tail -= 1
            ret_melody.append(melody[melody_idx[head] + 1 : melody_idx[tail] + 1])
            ret_lyric.append(lyric[lyric_idx[head] + 1 : lyric_idx[tail] + 1])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--lmd-data-dir", type=str, default="", help="Lmd data directory"
    )
    parser.add_argument(
